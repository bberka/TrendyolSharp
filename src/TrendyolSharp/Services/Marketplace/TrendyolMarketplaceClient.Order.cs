...
namespace TrendyolSharp.Services.Marketplace
{
  public partial class TrendyolMarketplaceClient
  {
    ...
    ///   <br /><br />
    ///   You can retrieve information about every order placed by customers, which corresponds to the products you have
    ///   submitted to the Trendyol system and is in payment control, using this method. After the payment control by the
    ///   system, the orders are automatically packaged, and order packages are created.
    ///   <br /><br />
    ///   If you do not use any parameter in the endpoint below, it returns you maximum one week order instead of all orders.
    ...
    public async Task<TrendyolApiResult<ResponseGetShipmentPackages>> GetShipmentPackagesAsync(FilterGetShipmentPackages filter = null) {
      ...
      url = url.BuildUrl(
                         ...
                         new KeyValuePair<string, object>("orderNumber", filter?.OrderNumber),
                         ...
                         new KeyValuePair<string, object>("shipmentPackageIds", filter?.ShipmentPackageIds)
                        );
    ...
    ///   <br /><br />
    ///   When this method is called for any package, it is now not through Trendyol's agreement but The status of the shipment
    ///   made by the supplier on its own agreement. The status of the shipment starts to be questioned and shipped, Delivered
    ///   information is received and tracked through the integration.
    ///   <br /><br />
    ///   If an order has been cancelled, Get Order Packages service must be used and the Updating Shipping Code must be made
    ///   to the current package number.
    ///   <br /><br />
    ///   If the package you are trying to feed with the cargo tracking number is in the status of cancelled, shipped,
    ///   delivered or if the tracking code has been fed to the relevant package before, you will receive the error "Shipment
    ///   Update Edilebilir Bir Durumda Değil.".
    ///   <br /><br />
    ///   The shipping code cannot be used for packages with the status of shipped, delivered.
    ///   <br /><br />
    ///   If a notification of unsupplied is made to a package whose cargo number has been updated, a new package and cargo
    ///   code will be created, so it is expected that the cargo code of the new package will be updated again through the
    ///   integration.
    ///   <br /><br />
    ///   If you are working with Yurtiçi Kargo, the courier code you have forwarded must be processed by YK. Otherwise, you
    ///   may get an "invalid tracking number"(400 Bad Request) error.
    /// </summary>
    ...
    ///   <br /><br />
    ///   It is used to notify Trendyol of the creation of the invoice of the order package. Notification of Invoice,a
    ///   reference to prevent customer-originated cancellations reaching to Trendyol Customer Care Service.
    ///   <br /><br />
    ///   You can only update the package of the order with 2 package status. Except for these statuses, the system is
    ///   automatically transferred to the package. You can find detailed information about the statuses below.
    ///   <br /><br />
    ///   While notifying the status, you should first notify "Picking" and then "Invoiced".
    ///   <br /><br />
    ///   Notify Packages as Picking: As soon as you notify the Picking status, the phrase "Order Processed" will be displayed
    ///   on the Trendyol panel. With this status, you can check the status of your orders on your side.
    ///   <br /><br />
    ///   Notify Packages as Invoiced: As soon as you notify the Invoiced status, the phrase "Order Processed" will be
    ///   displayed on the Trendyol panel. With this status, you can check the status of your orders on your side.
    /// </summary>
    /// <param name="id"></param>
    ...
    ///   <br /><br />
    ///   Each order package has a PackageID value. If one of the item is cancelled in the package,the package will be split
    ///   automatically and new PackageID will be generated and assigned to the new package.
    /// </summary>
    /// <param name="shipmentPackageId"></param>
    /// <param name="request"></param>
    /// <returns></returns>
    ...
    ///   If you cannot send the invoice for the relevant order by adding a PDF to the encrypted mail address in the order, you
    ///   have to use this service.
    ///   <br /><br />
    ///   By triggering this method Trendyol sends an e-invoice email to Trendyol Customer including the link provided by
    ///   Supplier
    ///   <br /><br />
    ...
    ///   <br /><br />
    ///   Each order package has a PackageID value. If one of the item is split in the package,the package status will be
    ///   "Unpack" automatically and new PackageID will be generated and assigned tothe new package. Therefore, you must get
    ///   orders with this endpoint
    ///   Dividing Order Packages with Multiple Barcodes
    ///   <br /><br />
    ///   After using this service, new packages depending on the order number will be generated asynchronously. For this
    ///   reason, you have to fetch the new generated packages from the getShipmentPackages again.
    ///   <br /><br />
    ///   With this method, you can process the products included in an order package with the quantity and orderLineId value
    ///   of the relevant barcode in the package.
    ///   <br /><br />
    ///   If there is a product(s) that you exclude while requesting, that product(s) will be created in a separate and new
    ///   package.
    /// </summary>
    /// <param name="shipmentPackageId"></param>
    /// <param name="request"></param>
    ...
    ///   https://developers.trendyol.com/docs/marketplace/siparis-entegrasyonu/siparis-paketlerini-bolme
    ///   <br /><br />
    ///   In this example, there will be 3 packages; First ,one package for 3,5,6 orderLines in the package. Second, another
    ///   package for 7,8,9 orderLines, and third, one package for the remaining orderLines.
    ///   <br /><br />
    ///   All orderLines in a package should not be sent to this service. The final package for the remaining orderlines will
    ///   be created by the system automatically.
    /// </summary>
    /// <param name="shipmentPackageId"></param>
    /// <returns></returns>
    ...
    ///   <br /><br />
    ///   With this service, you can feed deci and box quantity for your order packages of UPS Cargo and CEVA Logistics
    ///   companies.
    ///   <br /><br />
    ...
    ///   <br /><br />
    ///   You can forward these transactions to Trendyol with the following services, where alternative shipping options are
    ///   used to deliver the created order package to the customer.
    ///   <br /><br />
    ///   After receiving the 200 response, you can access the current cargoTrackingNumber value from the order packages
    ///   picking service and send this value to us after the customer receives the order with this value.
    /// </summary>
    /// <param name="id"></param>
    /// <param name="request"></param>
    /// <returns></returns>
    ...
    ///   <br /><br />
    ///   You can use this service for orders that are returned to your warehouse in the case of "shipped" status and cannot be
    ///   delivered to the customer, and therefore cannot be transferred to "delivered" status.
    ///   <br /><br />
    ///   It doesn't need a JSON body when making this request. After sending the request, you will receive a 200 OK response.
    /// </summary>
    ...
    ///   <br /><br />
    ///   Our sellers who integrate Supplier Pays with logistics companies and ship their products to the authorized service
    ///   should call this service. (Currently, we only work with Horoz Logistics among the logistics companies, and we have
    ///   supplier pays.)
    ///   <br /><br />
    ...
    ///   <br /><br />
    ///   This service will only be available for our sellers using Trendyol Express. With this service, the "WarehouseId"
    ///   field returned from the Picking Order Packages service will be updated.
    ///   <br /><br />
    ...
    ///   <br /><br />
    ///   Through this service, sellers can define additional supply times for order packages. If the
    ///   agreedDeliveryDateExtendible field returns true, the following service will be available. If the field returns false,
    ///   an error will be returned to you.
    ///   <br /><br />
    ///   There will be a specified date range for additional time to be entered. You can see this date range in order packages
    ///   picking service as agreedDeliveryExtensionStartDate, agreedDeliveryExtensionEndDate.
    ///   <br /><br />
